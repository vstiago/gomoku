...
class TVPlayer(Player):
    ...
    def __init__(self, symbol, board_size):
        ...
        for i in range(0, self._size):
            for j in range(0, self._size):
                self.update_scores(i, j)
        # self._sum_scores = np.full((self._size, self._size), 0.0)
        # for i in range(0, self._size):
        #     for j in range(0, self._size):
        #         self._sum_scores[i, j] = sum(self._scores[i, j])
    ...
    def new_score_me(self, board_slice, shift, me):
        ...
        return 10.0 * total_adjacent + 5.0 * counter + total_free

    def new_score(self, board_slice, shift):
        attack = self.new_score_me(board_slice, shift, self.MINE)
        defense = self.new_score_me(board_slice, shift, self.OPPONENT)
        return 1.1 * attack + defense
